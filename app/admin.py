...
async def adm_stats(c: CallbackQuery):
    ...
    async with db() as conn:
        ...
        owner = (await (await conn.execute("SELECT COALESCE(SUM(amount),0) FROM transactions WHERE user_id=0")).fetchone())[0]
    await c.message.edit_text(f"👥 Пользователи: {users}\n🖱 Клики: {clicks}\n🧾 Покупки: {orders}\n💰 Доход владельца: {owner/100:.2f} ₽")
    await c.answer()
...
async def adm_orders(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    async with db() as conn:
        cur = await conn.execute("SELECT id,buyer_id,offer_id,price,status FROM purchases WHERE status='pending' ORDER BY id DESC LIMIT 30")
...
async def confirm(m: Message):
    ...
    async with db() as conn:
        cur = await conn.execute("SELECT buyer_id, offer_id, price, status FROM purchases WHERE id=?", (pid,))
        row = await cur.fetchone()
        if not row:
            await m.answer("Заявка не найдена."); return
        buyer_id, offer_id, price, status = row
        if status != "pending":
            await m.answer("Эта заявка уже обработана."); return
        await conn.execute("UPDATE purchases SET status='delivered', code=? WHERE id=?", (code, pid))
        await conn.commit()
    await distribute_purchase_profit(buyer_id, int(price), offer_id)
    await m.bot.send_message(buyer_id, f"🎁 Покупка #{pid} подтверждена!\nТвой код: `{code}`", parse_mode="Markdown")
